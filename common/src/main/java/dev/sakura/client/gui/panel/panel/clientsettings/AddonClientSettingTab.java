...
import dev.sakura.client.gui.panel.adapter.SettingListController;
import dev.sakura.client.gui.panel.component.setting.KeybindSettingRow;
...
public final class AddonClientSettingTab implements ClientSettingTabView {
    ...
    private static final TranslateComponent idComponent = SakuraTranslateComponent.create("gui", "addon.info.id");
    private static final TranslateComponent versionComponent = SakuraTranslateComponent.create("gui", "addon.info.version");
    private static final TranslateComponent authorsComponent = SakuraTranslateComponent.create("gui", "addon.info.authors");
    private static final TranslateComponent modulesComponent = SakuraTranslateComponent.create("gui", "addon.info.modules");
    ...
    public void render(GuiGraphicsExtractor guiGraphics, PanelLayout.Rect bounds, int mouseX, int mouseY, float partialTick) {
        ...
        SakuraAddon selectedAddon = resolveSelectedAddon(addons);
        ...
        if (rebuildContent) {
            ...
            rowEntries.clear();
            List<String> addonIds = addons.stream().map(SakuraAddon::getAddonId).toList();
    ...
    private SakuraAddon resolveSelectedAddon(List<SakuraAddon> addons) {
        if (addons.isEmpty()) {
        ...
        SakuraAddon fallback = addons.getFirst();
    ...
    private void buildAddonListRow(PanelUiTree.Scope scope, SakuraAddon addon, PanelLayout.Rect rowBounds, float hoverProgress, float selectedProgress) {
    ...
    private void buildAddonInfo(PanelUiTree.Scope scope, SakuraAddon addon, PanelLayout.Rect infoBounds) {
    ...
    private PanelLayout.Rect getDetailInfoBounds(PanelLayout.Rect detailPanelBounds, SakuraAddon addon) {
        return new PanelLayout.Rect(
    ...
    private PanelLayout.Rect getDetailSettingsViewport(PanelLayout.Rect detailPanelBounds, SakuraAddon addon) {
    ...
    private float getDetailInfoHeight(PanelLayout.Rect detailPanelBounds, SakuraAddon addon) {
    ...
    private boolean shouldRebuild(PanelLayout.Rect bounds, int mouseX, int mouseY, List<SakuraAddon> addons, SakuraAddon selectedAddon, List<Setting<?>> selectedSettings, int guiHeight, long contentSignature) {
        ...
        List<String> addonKeys = addons.stream().map(SakuraAddon::getAddonId).toList();
    ...
    private void rememberSnapshot(PanelLayout.Rect bounds, int mouseX, int mouseY, List<SakuraAddon> addons, SakuraAddon selectedAddon, List<Setting<?>> selectedSettings, int guiHeight, long contentSignature) {
        ...
        lastAddonKeys = addons.stream().map(SakuraAddon::getAddonId).toList();
        lastVisibleSettings = selectedSettings.stream().map(Setting::getName).toList();
        lastContentSignature = contentSignature;
    }

    private long buildContentSignature(List<SakuraAddon> addons, SakuraAddon selectedAddon, List<Setting<?>> selectedSettings) {
        long signature = 17L;
