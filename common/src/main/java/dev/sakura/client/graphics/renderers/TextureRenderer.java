...
import dev.sakura.client.graphics.SakuraRenderPipelines;
import dev.sakura.client.graphics.SakuraRenderSystem;
import dev.sakura.client.graphics.SakuraTexture;
import dev.sakura.client.graphics.buffer.SakuraRingBuffer;
...
public class TextureRenderer implements IRenderer {
    ...
    public void addRoundedTexture(SakuraTexture texture, float x, float y, float width, float height, float radius, float u0, float v0, float u1, float v1, Color color) {
    ...
    public void addRoundedTexture(SakuraTexture texture, float x, float y, float width, float height, float radiusTL, float radiusTR, float radiusBR, float radiusBL, float u0, float v0, float u1, float v1, Color color) {
    ...
    private void addRoundedTexture(Object textureKey, float x, float y, float width, float height, float rTL, float rTR, float rBR, float rBL, float u0, float v0, float u1, float v1, Color color, boolean useLinearFilter) {
        Batch batch = batches.computeIfAbsent(textureKey, k -> {
            Batch b = new Batch(new SakuraRingBuffer(BUFFER_SIZE, GpuBuffer.USAGE_VERTEX));
    ...
    public void draw() {
        ...
        GpuTextureView colorView = SakuraRenderSystem.resolveColorView();
        ...
        for (Map.Entry<Object, Batch> entry : batches.entrySet()) {
            ...
            } else if (textureKey instanceof SakuraTexture tex) {
                texture = tex;
            } else {
                continue;
            }
            ...
            try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
                ...
                pass.setPipeline(SakuraRenderPipelines.TEXTURE);
    ...
    private SakuraTexture loadTexture(Identifier identifier, boolean useLinearFilter) {
        ...
        try {
            ...
            return new SakuraTexture(texture, view, sampler, false, false);
        ...
        return new SakuraTexture(texture, view, sampler, true, false);
    }
    ...
    private static final class Batch {
        final SakuraRingBuffer buffer;
        long currentOffset = 0;
        int vertexCount = 0;
        boolean useLinearFilter;

        private Batch(SakuraRingBuffer buffer) {
            this.buffer = buffer;
        }
    }
