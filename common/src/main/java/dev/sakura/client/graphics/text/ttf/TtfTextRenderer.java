package dev.sakura.client.graphics.text.ttf;
...
import dev.sakura.client.graphics.buffer.BufferUtils;
import dev.sakura.client.graphics.buffer.SakuraRingBuffer;
import dev.sakura.client.graphics.text.GlyphDescriptor;
...
public class TtfTextRenderer implements ITextRenderer {
    ...
    public void addText(String text, float x, float y, float scale, Color color, TtfFontLoader fontLoader) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            Batch batch = batches.computeIfAbsent(atlas, k -> new Batch(new SakuraRingBuffer(bufferSize, GpuBuffer.USAGE_VERTEX)));
            batch.buffer.tryMap();
    ...
    public void draw() {
        ...
        GpuTextureView depthView = SakuraRenderSystem.resolveDepthView();
        ...
        for (Map.Entry<TtfGlyphAtlas, Batch> entry : batches.entrySet()) {
            ...
            try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
                ...
                pass.setPipeline(SakuraRenderPipelines.TTF_FONT);
                if (scissorEnabled) {
